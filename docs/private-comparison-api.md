...
1. 官方数据存在 Product ID 时，以 Product ID 作为跨地区唯一身份；其他地区缺少相同 Product ID 时按缺失处理，不退回同名套餐。
2. 官方数据同时提供结构化计费周期时，优先使用 `MONTHLY`、`YEARLY`、`WEEKLY` 或 ISO 8601 的 `P1M`、`P1Y`、`P1W`。
3. 官方公开页面没有 Product ID 时，继续采用统一 `?l=en` 后的英文名称和同名出现顺序匹配，不做跨语言或模糊猜测。
4. 无法可靠识别周期的同名套餐使用 `#2`、`#3` 区分，不根据价格强行归类。

私有 API 的套餐对象可包含：

- `productId`：仅在上游官方数据确实提供时出现。
- `matchMethod`：`product-id` 或 `name-occurrence`，供排错和后续升级使用，不需要展示给公众号用户。
